...
class SimeisSDK:
    ...
    def wait_until_ship_idle(self, ship_id, ts=1, max_wait=180, autotick=True):
        start = time.time()
        ship = self.get(f"/ship/{ship_id}")
        state = self._state_name(ship.get("state"))

        while state != "Idle":
            # In testing mode, the game progresses only when /tick is called.
            if autotick:
                try:
                    self.post("/tick")
                except Exception:
            ...
            if max_wait is not None and (time.time() - start) >= max_wait:
                raise SimeisError(
                    f"Timeout waiting ship {ship_id} to be idle (last state: {state})"
                )
